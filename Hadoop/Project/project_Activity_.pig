-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/AnilPothuganti/input_files/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/AnilPothuganti/input_files/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/AnilPothuganti/input_files/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
STORE namesOrdered INTO 'hdfs:///user/AnilPothuganti/output_files/pigoutput' USING PigStorage('\t');
